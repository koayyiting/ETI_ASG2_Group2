...
func main() {
	db, err = sql.Open("mysql", "user:password@tcp(127.0.0.1:2479)/studentDB")
...
func createStudent(w http.ResponseWriter, r *http.Request) {
	...
	stmt, err := db.Prepare("INSERT INTO Student (StudentID, StudentFirstName, StudentLastName, Phone, StudentEmail, Password) VALUES (?, ?, ?, ?, ?, ?)")
	if err != nil {
	...
	_, err = stmt.Exec(newStudent.StudentID, newStudent.StudentFirstName, newStudent.StudentLastName, newStudent.PhoneNo, newStudent.StudentEmail, newStudent.Password)
	if err != nil {
		http.Error(w, "Server Error 500", http.StatusInternalServerError)
		return
	}
...
func getStudent(w http.ResponseWriter, r *http.Request) {
	...
	var acc Student
	err := db.QueryRow("SELECT StudentID, StudentFirstName, StudentLastName, Phone, StudentEmail, Password FROM Student WHERE StudentEmail = ? AND Password = ?", email, password).Scan(&acc.StudentID, &acc.StudentFirstName, &acc.StudentLastName, &acc.PhoneNo, &acc.StudentEmail, &acc.Password)
...
func updateStudent(w http.ResponseWriter, r *http.Request) {
	...
	stmt, err := db.Prepare("UPDATE Student SET StudentFirstName=?, StudentLastName=?, PhoneNo=?, StudentEmail=?, Password=? WHERE StudentID=?")
	if err != nil {
		http.Error(w, "Server Error 500", http.StatusInternalServerError)
		return
	}
	...
	_, err = stmt.Exec(updStudent.StudentFirstName, updStudent.StudentLastName, updStudent.PhoneNo, updStudent.StudentEmail, updStudent.Password, StudentID)
	if err != nil {
		http.Error(w, "Server Error 500", http.StatusInternalServerError)
		return
	}
